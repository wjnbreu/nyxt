...
(cffi:defcallback clear-do-nothing-callback
    :void ((source-object :pointer) (result :pointer) (user-data :pointer))
  (declare (ignore source-object))
  (let ((callback (find (cffi:pointer-address user-data) webkit::callbacks
                        :key (function webkit::callback-id))))
    (handler-case
        (let* ((clear-result (webkit:webkit-website-data-manager-clear-finish
                              (webkit:webkit-web-view-get-website-data-manager
                               (webkit::callback-web-view callback))
                              result)))
          (declare (ignore clear-result))
          (setf webkit::callbacks (delete callback webkit::callbacks))))))

(define-ffi-method ffi-buffer-clear-cache ((buffer gtk-buffer))
  (incf webkit::callback-counter)
  (push (webkit::make-callback :id webkit::callback-counter :web-view (gtk-object buffer))
        webkit::callbacks)
  (webkit:webkit-web-context-clear-cache
   (webkit:webkit-web-view-web-context (gtk-object buffer)))
  (webkit:webkit-website-data-manager-clear
   (webkit:webkit-web-view-get-website-data-manager (gtk-object buffer))
   :webkit-website-data-all
   0
   (cffi:null-pointer)
   (cffi:callback clear-do-nothing-callback)
   (cffi:make-pointer webkit::callback-counter)))
